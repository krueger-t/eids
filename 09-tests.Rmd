...
- **Beispiel 1**: Laut [dieser Umfrage](https://www.personalwirtschaft.de/der-job-hr/arbeitswelt/artikel/pendelstrecken-werden-immer-laenger.html) hält jeder zweite Berufspendler eine durchschnittliche Fahrtzeit von bis zu 60 min pro Strecke für akzeptabel. Anhand der Reisezeitdaten der Studierenden aus dem Wintersemester 2021/22 (Abbildung \@ref(fig:freisezeit)) wollen wir fragen: Ist der mit dem Stichprobenmittel $\bar x=55.4$ geschätzte Mittelwert $\mu$ der Grundgesamtheit _kleiner_ als der Vergleichswert $\mu_0=60$ aus der Studie? Beziehungsweise, ist der _Unterschied statistisch signifikant_, wenn wir die Streuung der Stichprobe berücksichtigen? Die Frage "kleiner als" wenn es um einen Mittelwert geht beantwortet der sogenannte **linksseitige Einstichproben-t-Test** (Kapitel \@ref(ttest1)).

```{r freisezeit, echo=FALSE, fig.align='center', fig.cap='Histogramm des Merkmals "Reisezeit" der Reisedaten aus dem Wintersemester 2021/22. Die vertikale Linie markiert den Mittelwert von 55.4 min.', out.width='50%'}
...
h <- hist(reisedat$zeit, breaks = seq(0, 130, 10), plot = FALSE)
# absolute in relative Häufigkeiten umrechnen
h$counts <- h$counts / sum(h$counts)
# Histogrammdaten plotten
plot(h, freq = TRUE, col = "gray", ylim = c(0,0.4),
     main = "", xlab = "Reisezeit (min)", ylab = "relative Häufigkeit")
lines(c(1,1)*mean(reisedat$zeit, na.rm=TRUE), c(0, 0.4), col='black')
```

- **Beispiel 2**: Ist der mit dem Stichprobenmittel $\bar x=55.4$ geschätzte Mittelwert $\mu$ der Grundgesamtheit _ungleich_ dem Vergleichswert $\mu_0=60$ aus der Studie? Wenn wir die Frage so formulieren brauchen wir einen sogenannten **zweiseitigen Einstichproben-t-Test** (Kapitel \@ref(ttest1)).
- **Beispiel 3**: Laut [derselben Umfrage](https://www.personalwirtschaft.de/der-job-hr/arbeitswelt/artikel/pendelstrecken-werden-immer-laenger.html) nehmen 21% der Pendler eine Fahrtzeit zwischen 30 und 45 min in Kauf. Ist der mit dem Stichprobenmittel $\bar x=55.4$ geschätzte Mittelwert $\mu$ der Grundgesamtheit _größer_ als der Vergleichswert $\mu_0=45$ aus der Studie? Beziehungsweise, ist der _Unterschied statistisch signifikant_, wenn wir die Streuung der Stichprobe berücksichtigen? Die Frage "größer als" beantwortet der **rechtsseitige Einstichproben-t-Test** (Kapitel \@ref(ttest1)).^[Die Fragestellungen sind etwas konstruiert, damit wir die gängigsten Versionen des t-Tests kennenlernen, sind aber halbwegs realistisch.]
- **Beispiel 4**: Die Stichprobe wurde zufällig geteilt (Abbildung \@ref(fig:freisezeit2)). Sind die mit den neuen Stichprobenmitteln $\bar x_1=48.5$ und $\bar x_2=62.0$ geschätzten Mittelwerte $\mu_1$ und $\mu_2$ ungleich? Beziehungsweise, können wir statistisch nachvollziehen, dass die beiden Stichproben Grundgesamtheiten mit dem selben Mittelwert entstammen? Diese Frage beantwortet der sogenannte **Zweistichproben-t-Test (zweiseitig)** (Kapitel \@ref(ttest2)).

```{r freisezeit2, echo=FALSE, fig.align='center', fig.cap='Histogramme des Merkmals "Reisezeit" der Reisedaten aus dem Wintersemester 2021/22. Die Stichprobe wurde zufällig geteilt. Die vertikalen Linien markieren die Mittelwerte $\\bar x_1=48.5$ (links) und $\\bar x_2=62.0$ (rechts). Die Varianzen sind $s_1^2=221$ und $s_2^2=643$.', fig.show='hold', out.width='50%'}
# "seed" wählen, so dass Zufallszahlen reproduziert werden können
set.seed(20210105)
# zufällig die Hälfte der Reisezeitdaten von 2021 auswählen
loc <- sample(seq(1,61,1), 30, replace = FALSE)
# Teilstichprobe 1
t1 <- reisedat$zeit[loc]
# Teilstichprobe 2 (Komplementärmenge)
t2 <- reisedat$zeit[!is.element(seq(1,61,1),loc)]
# Histogramme
h1 <- hist(t1, breaks = seq(0, 130, 10), plot = FALSE)
h1$counts <- h1$counts / sum(h1$counts)
plot(h1, freq = TRUE, col = "gray", ylim = c(0,0.4),
     main = "", xlab = "Reisezeit (min)", ylab = "relative Häufigkeit")
lines(c(1,1)*mean(t1, na.rm=TRUE), c(0, 0.4), col='black')
h2 <- hist(t2, breaks = seq(0, 130, 10), plot = FALSE)
h2$counts <- h2$counts / sum(h2$counts)
plot(h2, freq = TRUE, col = "gray", ylim = c(0,0.4),
     main = "", xlab = "Reisezeit (min)", ylab = "relative Häufigkeit")
lines(c(1,1)*mean(t2, na.rm=TRUE), c(0, 0.4), col='black')
```
...
- **Beispiel 6**: Zurück zu den beiden neuen Stichproben aus Beispiel 4 (Abbildung \@ref(fig:freisezeit2)). Ist die Varianz $\sigma_2^2$ (gegeben $s_2^2=643$) größer als die Varianz $\sigma_1^2$ (gegeben $s_1^2=221$)? Beziehungsweise, können wir statistisch nachvollziehen, dass die beiden Stichproben Grundgesamtheiten mit derselben Varianz entstammen? Diese Frage beantwortet der sogenannte **F-Test (rechtsseitig)** (Kapitel \@ref(ftest)).
...
> Bsp. 3: Ist $\mu$ (gegeben $\bar x=55.4$) größer als $\mu_0=45$?
...
> Bsp. 2: Ist $\mu$ (gegeben $\bar x=55.4$) ungleich $\mu_0=60$?

Bei **einseitigen** Tests wird auf _kleiner/nicht kleiner_ oder _größer/nicht größer_ getestet.

> Bsp. 1: Ist $\mu$ (gegeben $\bar x=55.4$) kleiner als $\mu_0=60$?
...
Jeder Test hat eine bestimmte **Teststatistik** (Prüfwert) von der wir wissen, wie sie bei wiederholtem Stichprobenziehen verteilt ist (unter bestimmten Annahmen), _falls die Nullhypothese wahr ist_.^[Genau genommen bezieht sich das auf die Nullhypothese im zweiseitigen Fall, z.B. $H_0: \mu =\mu_0$ im Fall des t-Tests. Das liegt daran, dass wir nur für den Fall "=" eine theoretische Wahrscheinlichkeitverteilung aufstellen können.] Die Teststatistik eines Einstichproben-t-Tests beispielsweise ist:
$$\begin{equation}
t_s=\frac{\hat\mu-\mu_0}{s_{\hat\mu}}\sim t_{n-1}
(\#eq:ts)
\end{equation}$$

$\hat\mu$ ist der Mittelwertschätzer; in Bsp. 1 $\hat\mu=\bar x=55.4$. $\mu_0$ ist der Wert, mit dem wir den Schätzer vergleichen; in Bsp. 1 $\mu_0=60$. $s_{\hat\mu}$ ist der Standardfehler des Mittelwertschätzers. Ist die Grundgesamtheit normalverteilt mit $\mu=\mu_0$, dann ist der so standardisierte Schätzer des Mittelwertes (die Teststatistik $t_s$) bei wiederholtem Stichprobenziehen t-verteilt mit $n-1$ Freiheitsgraden (vgl. Kapitel \@ref(schaetzen)).^[In der Tat hat der t-Test seinen Namen von der t-Verteilung seiner Teststatistik.]

### Was genau getestet wird

Wenn die Teststatistik nahe dem Zentrum der Verteilung ist, die unter der Nullhypothese zu erwarten ist, d.h. in einem Bereich hoher Wahrscheinlichkeit, dann lehnen wir die Nullhypothese _nicht_ ab. In Abbildung \@ref(fig:testprinzip) ist das für die Teststatistik $t_s$ in blau und die t-Verteilung dargestellt. Im Bsp. 2 würden wir in so einem Fall bis auf weiteres schließen, dass $\mu$ (gegeben $\bar x=55.4$) gleich $\mu_0=60$ ist. Das ist in dem Beispiel tatsächlich das Ergebnis - wie wir noch sehen werden.

Ist die Teststatistik dagegen in den Extremen der Verteilung, d.h. in einem Bereich geringer Wahrscheinlichkeit, dann lehnen wir die Nullhypothese ab. In Abbildung \@ref(fig:testprinzip) ist das mit den roten Pfeilen verdeutlicht. Im Bsp. 2 würden wir in so einem Fall schließen, dass $\mu$ (gegeben $\bar x=55.4$) ungleich $\mu_0=60$ ist.
...
> Ist der mit dem Stichprobenmittel $\bar x=55.4$ geschätzte Mittelwert $\mu$ der Grundgesamtheit _kleiner_ als der Vergleichswert $\mu_0=60$? Beziehungsweise, ist der _Unterschied statistisch signifikant_, wenn wir die Streuung der Stichprobe berücksichtigen?
...
Setzen wir die Zahlenwerte aus der Stichprobe ein ("reisedat$zeit" enthält die Merkmalswerte für "Reisezeit" aus dem Wintersemester 2021/22):
```{r echo=TRUE}
# Mittelwert
# na.rm=TRUE ignoriert NAs
xbar <- mean(reisedat$zeit, na.rm=TRUE)
xbar
# Vergleichswert
mu0 <- 60
# Standardabweichung
# na.rm=TRUE ignoriert NAs
s <- sd(reisedat$zeit, na.rm=TRUE)
s
# Stichprobenumfang
# !is.na(reisedat$zeit) verweist auf die Werte, die nicht NA sind
n <- length(reisedat$zeit[!is.na(reisedat$zeit)])
n
# Teststatistik
...
Dieser Wert der Teststatistik liegt nicht direkt in den Extremen der t-Verteilung, die unter der Nullhypothese zu erwarten ist, aber auch nicht direkt im Zentrum:
...
> Ist der mit dem Stichprobenmittel $\bar x=55.4$ geschätzte Mittelwert $\mu$ der Grundgesamtheit _ungleich_ dem Vergleichswert $\mu_0=60$?

Die Nullhypothese ist in diesem Fall, dass der Mittelwert gleich dem Vergleichswert ist:^[Genau genommen ist es diese Nullhypothese, also der zweiseitige Fall, für die die Wahrscheinlichkeitsverteilung der Teststatistik gilt. Für den einseitigen Fall wird die Nullhypothese trotzdem üblicherweise als "$\leq$" bzw. "$\geq$" angegeben.]
$$H_0:\mu=\mu_0$$
...
> Es kann nicht ausgeschlossen werden, dass der mit dem Stichprobenmittel $\bar x$ geschätzte Mittelwert $\mu$ der Grundgesamtheit gleich dem Vergleichswert $\mu_0=60$ ist.
...
> Ist der mit dem Stichprobenmittel $\bar x=55.4$ geschätzte Mittelwert $\mu$ der Grundgesamtheit _größer_ als der Vergleichswert $\mu_0=45$? Beziehungsweise, ist der _Unterschied statistisch signifikant_, wenn wir die Streuung der Stichprobe berücksichtigen?
...
Der Wert der Teststatistik ist jetzt positiv, da $\bar x$ größer ist als $\mu_0$. Er liegt in den Extremen der t-Verteilung, die unter der Nullhypothese zu erwarten ist, nur eben auf der rechten Seite:
```{r echo=TRUE, out.width='80%'}
plot(seq(-5,5,0.01), pt(seq(-5,5,0.01), n-1), ylim=c(0,1), type='l',
     xlab='Z=t_s', ylab='Verteilungsfunktion')
lines(c(1, 1)*ts, c(0, pt(ts, n-1)), col='blue')
...
> Sind die mit den Stichprobenmitteln $\bar x_1=48.5$ und $\bar x_2=62.0$ geschätzten Mittelwerte $\mu_1$ und $\mu_2$ ungleich? Beziehungsweise, können wir statistisch nachvollziehen, dass die beiden Stichproben Grundgesamtheiten mit dem selben Mittelwert entstammen?
...
Auch in die Anzahl Freiheitsgrade der t-Verteilung, die unter der Nullhypothese zu erwarten ist, gehen beide Stichprobenumfänge ein. Setzen wir die Zahlenwerte aus den Stichproben ein ("t1" und "t2" stehen für die Merkmalswerte für "Reeisezeit" der ersten bzw. zweiten Stichprobe):
```{r echo=TRUE}
# Mittelwerte
# na.rm=TRUE ignoriert NAs
...
Dieser Wert der Teststatistik liegt tendenziel in den Extremen der t-Verteilung, die unter der Nullhypothese zu erwarten ist:
```{r echo=TRUE, out.width='80%'}
...
Also wieder halb so groß wie der zweiseitige p-Wert. Der p-Wert ist kleiner als das konventionelle Signifikanzniveau von 0.01, d.h. es ist sehr wahrscheinlich, dass ein solcher Wert der Teststatistik durch Zufall zustande kommt falls die Nullhypothese wahr ist, d.h. wir lehnen die Nullhypothese ab. Dies ist so obwohl wir wissen, dass beide Stichproben der selben Grundgesamtheit entstammen - wir können diesen Tatbestand nur nicht _statistisch_ nachweisen! Der Grund ist wahrscheinlich, dass die Teilstichproben zu klein sind.

Für Beispiel 4 schließen wir also:

> Der Unterschied der mit den Stichprobenmitteln $\bar x_1$ und $\bar x_2$ geschätzten Mittelwerte $\mu_1$ und $\mu_2$ ist _signifikant_. D.h. wir können statistisch nicht nachweisen, dass die beiden Stichproben Grundgesamtheiten mit dem selben Mittelwert entstammen.
...
> Ist die Varianz $\sigma_2^2$ (gegeben $s_2^2=643$) größer als die Varianz $\sigma_1^2$ (gegeben $s_1^2=221$)? Beziehungsweise, können wir statistisch nachvollziehen, dass die beiden Stichproben Grundgesamtheiten mit derselben Varianz entstammen?

Die Nullhypothese ist in diesem Fall, dass die beiden Varianzen gleich sind:
$$H_0:\sigma_2^2=\sigma_1^2$$
Die Alternativhypothese ist, dass $\sigma_2^2$ _größer_ ist als $\sigma_1^2$:
$$H_1:\sigma_2^2>\sigma_1^2$$

Die Alternativhypothese ergibt sich wie beim t-Test aus den Zahlenwerten der Stichproben, wo $s_2^2$ tatsächlich größer ist als $s_1^2$. Die Formulierung ist immer die eines rechtsseitigen Tests, wo die größere Varianz links steht.^[Trotzdem wird die Nullhypothese üblicherweise mit "=" angegeben, was zwar der Fall ist, für den die Wahrscheinlichkeitsverteilung der Teststatistik gilt, was aber nicht konsistent mit der Formulierung der Nullhypothese beim einseitigen t-Test ist. Für diese Inkonsistenz konnte ich bisher keine Lösung finden.] Die Teststatistik $F_s$ ist:
$$\begin{equation}
F_s=\frac{s_2^2}{s_1^2}\sim F_{n_1-1;n_2-1}
(\#eq:fs)
\end{equation}$$

Die größere Stichprobenvarianz steht für den rechtsseitigen Test immer im Zähler.^[Das hat wahrscheinlich numerische Gründe: Seht die größere Stichprobenvarianz immer im Zähler, dann nimmt die Teststatistik nur Werte größer oder gleich 1 an. So werden Rundungsfehler bei Zahlen kleiner 1 vermieden, die stärker ins Gewicht fallen würden.] Sind die beiden Stichprobenvarianzen annähernd gleich dann ist $F_s$ annähernd $1$. Diese Teststatistik folgt bei wiederholtem Stichprobenziehen, falls die Nullhypothese wahr ist, einer sogenannten **F-Verteilung** mit den Parameterwerten $n_1-1$ und $n_2-1$ (auch hier genannt "Freiheitsgrade", wobei es im Gegensatz zur t-Verteilung zwei gibt).^[Auch hier gibt die Verteilung der Teststatistik dem Test seinen Namen.] Abbildung \@ref(fig:fpdfcdfvariation) zeigt die F-Verteilung für verschiedene Kombinationen der beiden Freiheitsgrade. Die F-Verteilung geht von $0$ bis $+\infty$, d.h. ist nur für positive Werte definiert. Das passt für Varianzen, da diese nie negativ sind. Die F-Verteilung wird schmaler mit zunehmender Anzahl Freiheitsgrade, d.h. mit zunehmenden Stichprobengrößen $n_1-1$ und $n_2-1$ im Falle des F-Tests.
...
Dieser Wert der Teststatistik liegt in den Extremen der F-Verteilung, die unter der Nullhypothese zu erwarten ist:
```{r echo=TRUE, out.width='80%'}
...
text(fs,-0.2,"F_s", col="blue", xpd=TRUE)
```
...
Da der p-Wert viel kleiner ist als das konventionelle Signifikanzniveau von 0.01, ist sehr wahrscheinlich, dass ein solcher Wert der Teststatistik durch Zufall zustande kommt falls die Nullhypothese wahr ist. Wir lehnen demnach die Nullhypothese ab und schließen für Beispiel 6:

> Der Unterschied der beiden Varianzen ist statistisch _signifikant_. D.h. wir konnten nicht nachweisen, dass die beiden Stichproben Grundgesamtheiten mit der selben Varianz entstammen.

Das ist analog zum Bsp. 4; statistisch konnten wir nicht nachvollziehen, dass die beiden Stichproben der selben Grundgesamtheit entstammen. Das liegt, wie gesagt, vermutlich an der geringen Teilstichprobengröße.
...
Die Nullhypothese ist in diesem Fall, dass die Verteilungsfunktion der Zufallsvariable - nennen wir sie $X$ - gleich einer bestimmten Verteilungsfunktion $F_0$ - hier einer Normalverteilung - ist:
$$H_0:F_X(x)=F_0(x)$$
...
plot(h, freq = FALSE, col = "gray",
     main = "", xlab = "Reisezeit (min)", ylab = "Dichtefunktion")
...
dn
# KS-Test
ks.test(reisedat$zeit, "pnorm", xbar, s)
```

Der Wert der Teststatistik $d_n$ ist 0.18; wir haben ihn "per Hand" ausgerechnet als auch als Ausgabe "D" des KS-Testes erhalten. Die Warnung bezüglich der gleichen Stichprobenwerte ("ties") ist unvermeidlich bei realen Stichproben und für unsere Zwecke zu vernachlässigen. Da der p-Wert größer ist als das konventionelle Signifikanzniveau von 0.01, ist wahrscheinlich, dass dieser Wert der Teststatistik durch Zufall zustande kam falls die Nullhypothese wahr ist. Wir können demnach die Nullhypothese _nicht_ ablehnen und schließen für Beispiel 7:

> Die Verteilungen sind statistisch gleich. D.h. wir können nicht ausschließen, dass die Stichprobe der Reisezeit einer normalverteilten Grundgesamtheit entstammt, obwohl die empirische Verteilung steiler und rechtsschief aussieht.
...
Die Nullhypothese ist in diesem Fall, dass die Verteilungsfunktionen der beiden Zufallsvariablen - nennen wir sie $X_1$ und $X_2$ - gleich sind:
...
```{r ks2, echo=FALSE, fig.align='center', fig.cap='Empirische Verteilungsfunktionen der beiden Teilstichproben des Merkmals "Reisezeit" vom WiSe 2021//22.', message=FALSE, warning=FALSE}
...
Der Wert der Teststatistik $d_n$ ist diesmal 0.32; diesmal können wir ihn wegen der unterschiedlichen Stichprobengrößen nicht so einfach "per Hand" ausrechnen und sparen uns das hier. Da der p-Wert wiederum größer ist als das konventionelle Signifikanzniveau von 0.01, können wir die Nullhypothese _nicht_ ablehnen und schließen für Beispiel 8:

> Die beiden Teilstichproben entstammen einer gemeinsamen Verteilungen. Bzw., wir können diesen Tatbestand statistisch nachvollziehen.

In der Tat wurden die beiden Teilstichproben ja der gleichen Stichprobe zufällig entnommen. Hier zeigt sich aber einmal mehr, wie insensitiv der KS-Test ist; die zwei empirischen Verteilungen sehen sehr unterschiedlich aus (und gemeinsamen Mittelwert und Varianz konnten wir nicht nachweisen), trotzdem laesst der KS-Test die Nullhypothese einer gemeinsamen Verteilung nicht ablehnen.

## Unabhängigkeitstest (Chi-Quadrat-Test) {#chi2test}

Der Chi-Quadrat-Test testet die Unabhängigkeit zweier nominal skalierter Merkmale mit Hilfe der Chi-Quadrat-Statistik $\mathcal{X}^2$, die wir bereits in Kapitel \@ref(korrelation) anhand der Kontingenztabelle kennengelernt haben. Erinnern wir uns, dass $\mathcal{X}^2$ die Stärke des Zusammenhangs der Merkmale misst, ohne aber eine Aussage über Signifkanz zu treffen.
...
Die Chi-Quadrat-Statistik, die wir in Kapitel \@ref(korrelation) ausgerechnet haben ist die Teststatistik des Chi-Quadrat-Testes:
$$\begin{equation}
\mathcal{X}^2=\sum_{i=1}^{k}\sum_{j=1}^{m}\frac{\left(h_{ij}-\tilde h_{ij}\right)^2}{\tilde h_{ij}}
(\#eq:chi2)
\end{equation}$$

In Worten: Die Summe der relativen quadratischen Abweichungen der beobachteten Häufigkeiten $h_{ij}$ von den bei Unabhängigkeit erwarteten Häufigkeiten $\tilde h_{ij}$. In unserem Beispiel ist $\mathcal{X}^2=49895.1$. Diese Teststatistik folgt bei wiederholtem Stichprobenziehen, falls die Nullhypothese wahr ist, einer **Chi-Quadrat-Verteilung** mit Parameterwert $(k-1)\cdot(m-1)$ (auch hier genannt "Anzahl Freiheitsgrade").^[Wiederum ist es die Verteilung, die Teststatistik und Test ihren Namen gibt.] Abbildung \@ref(fig:chi2pdfcdfvariation) zeigt die Chi-Quadrat-Verteilung für verschiedene Anzahl Freiheitsgrade. Die Chi-Quadrat-Verteilung ist wie die F-Verteilung nur für positive Werte definiert. Das passt für $\mathcal{X}^2$, das nie negativ ist. Die Chi-Quadrat-Verteilung wird breiter mit zunehmender Anzahl Freiheitsgrade, d.h. mit zunehmender Anzahl Ausprägungen der beiden Merkmale. Das trägt dem mit zunehmender Anzahl Merkmalsausprägungen größer werdenden Maximalwert von  $\mathcal{X}^2$ Rechnung.
...
Somit ist es sehr unwahrscheinlich, dass dieser Wert der Teststatistik durch Zufall zustande kam falls die Nullhypothese wahr ist. Wir lehnen demnach die Nullhypothese ab und schließen für Beispiel 9:
