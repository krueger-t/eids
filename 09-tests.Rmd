...
Ist die Teststatistik dagegen in den Extremen der Verteilung, d.h. in einem Bereich geringer Wahrscheinlichkeit, dann lehnen wir die Nullhypothese ab. In Abbildung \@ref(fig:testprinzip) ist das mit den roten Pfeilen verdeutlicht. Im Bsp. 2 würden wir in so einem Fall schließen, dass die Daten nicht mit der Nullhypothese $\mu=\mu_0=60$ kompatibel sind. Das ist in dem Beispiel das Ergebnis, wie wir noch sehen werden.
...
Der p-Wert ist sehr klein, d.h. es ist sehr unwahrscheinlich, dass dieser Wert der Teststatistik durch Zufall zustande kam, falls die Nullhypothese wahr ist. Aber ist er unwahrscheinlich genug, um die Nullhypothese abzulehnen? In der klassischen Statistik entscheiden wir das auf Basis eines sogenannten **Signifikanzniveaus**, z.B. 0.01: Ist der p-Wert kleiner oder gleich 0.01, lehnen wir die Nullhypothese ab. Ist der p-Wert größer als 0.01, behalten wir die Nullhypothese bis auf weiteres bei. Das Signifikanzniveau von 0.01 ist dabei reine Konvention! Tatsächlich ist die binäre Einteilung andhand des p-Wertes in signifikant/nicht signifikant seit längerem in der Kritik (z.B. @amrhein2019). _R_ beispielsweise gibt Signifikanz zu mehreren Niveaus an. Grundsätzlich ist immer der p-Wert anzugeben. Dann kann jede Person ihr eigenes Signifikanzniveau ansetzen.

Für unser Beispiel 1 schließen wir jedenfalls unter diesen Bedingungen:

> Die Daten sind nicht kompatibel mit der Nullhypothese $\mu=\mu_0=60$. Der Unterschied zum geschätzten Mittelwert $\bar x=$ `r round(mean(reisedat$zeit_morgens, na.rm=TRUE),1)` ist groß vor dem Hintergrund des Standardfehlers des Schätzers, der sich wiederum aus der Streung der Stichprobe ergibt. Der Unterschied ist "statistisch signifikant" in der klassischen Interpretation.
...
> Die Daten sind nicht kompatibel mit der Nullhypothese $\mu=\mu_0=60$.
...
Die Nullhypothese ist wieder, dass der Mittelwert gleich dem Vergleichswert ist:^[Wie im linksseitigen Fall findet man in Textbüchern und online Quellen oft die Formulierung $H_0:\mu\leq\mu_0$, die sich aber nicht mit den wahrscheinlichkeitstheoretischen Überlegungen zur Nullhypothese vereinbaren lässt, wie wir noch sehen werden.]
$$H_0:\mu=\mu_0$$
...
Der Wert der Teststatistik ist jetzt positiv, da $\bar x$ größer ist als $\mu_0$. Er liegt wieder in den Extremen der t-Verteilung, die unter der Nullhypothese zu erwarten ist, und zwar auf der rechten Seite:
```{r echo=TRUE, out.width='80%'}
plot(seq(-5,5,0.01), pt(seq(-5,5,0.01), n-1), ylim=c(0,1), type='l',
     xlab='Z=t_s', ylab='Verteilungsfunktion')
lines(c(1, 1)*ts, c(0, pt(ts, n-1)), col='blue')
...
Dieser p-Wert ist wieder wesentlich kleiner als das konventionelle Signifikanzniveau von 0.01, d.h. wir lehnen diese Nullhypothese ebenfalls ab und schließen für Beispiel 3:
...
Ein p-Wert von 0.01 sagt aus, dass wir bei _hypothetisch wiederholter Stichprobenziehung_ aus der selben Grundgesamtheit in 1% der Fälle die Nullhypothese ablehnen würden obwohl sie wahr ist - ein sogenannter **Fehler 1. Art**.
...
> Ist die Varianz der Reisezeit nachmittags $\sigma_2^2$ (gegeben $s_2^2=$ `r round(var(reisedat$zeit_nachmittags, na.rm=TRUE),0)`) größer als die Varianz der Reisezeit morgens $\sigma_1^2$ (gegeben $s_1^2=$ `r round(var(reisedat$zeit_morgens, na.rm=TRUE),0)`)?
...
Die Alternativhypothese ergibt sich wie beim t-Test aus den Zahlenwerten der Stichproben, wo $s_2^2$ tatsächlich größer ist als $s_1^2$. Die Formulierung ist per Konvention immer die eines rechtsseitigen Tests, wo die größere Varianz links steht. Die Teststatistik $F_s$ ist:
$$\begin{equation}
...
Da der p-Wert größer ist als das konventionelle Signifikanzniveau von 0.01, ist es wahrscheinlich, dass dieser Wert der Teststatistik durch Zufall zustande kam falls die Nullhypothese wahr ist. Wir lehnen demnach die Nullhypothese nicht ab und schließen für Beispiel 6:
...
| :---: | :---: | :---: | :---: | :---: | :---: |
| Faktor | $SSA=\\SSY-SSE$ | $k-1$ | $\frac{SSA}{df_{SSA}}$ | $\frac{\frac{SSA}{df_{SSA}}}{s^2}$ | $1-F\left(F_s,k-1,n-k\right)$ |
...
Da der p-Wert größer ist als das konventionelle Signifikanzniveau von 0.01, ist es wahrscheinlich, dass dieser Wert der Teststatistik durch Zufall zustande kam falls die Nullhypothese wahr ist. Wir können demnach die Nullhypothese _nicht_ ablehnen und schließen für Beispiel 7:
...
In diesem Beispiel testen wir auf eine Normalverteilung, grundsätzlich können wir aber auf jede andere Verteilung testen. Die Teststatistik - hier genannt $d_n$ - ist im Fall des KS-Tests der maximale absolute Abstand zwischen empirischer und theoretischer Verteilungsfunktion:
$$\begin{equation}
...
Diese Teststatistik folgt bei wiederholtem Stichprobenziehen, falls die Nullhypothese wahr ist, einer sogenannten **Kolmogorow-Verteilung**. Diese ist in _R_ nicht explizit implementiert und generell aufwendig zu berechnen. Deshalb zeigen wir hier keine Darstellung, sondern benutzen sofort den in _R_ implementierten KS-Test. Das Prinzip ist aber dasselbe wie bei t- und F-Test und allen anderen Tests. Die Teststatistik können wir weiterhin "per Hand" ausrechnen:
...
t_sort <- sort(reisedat$zeit_morgens)
#  empirische Verteilungsfunktion
cdf_e <- seq(0+1/length(t_sort), 1, 1/length(t_sort))
#  theoretische Verteilungsfunktion
cdf_t <- pnorm(t_sort, mean=xbar, sd=s)
# absolute Differenzen
diff <- abs(cdf_e-cdf_t)
...
Der Wert der Teststatistik $d_n$ ist `r round(dn,2)`; wir haben ihn "per Hand" ausgerechnet als auch als Ausgabe "D" des KS-Tests erhalten. Die Warnung bezüglich gleicher Stichprobenwerte ("ties") ist unvermeidlich bei realen Stichproben und für unsere Zwecke zu vernachlässigen. Da der p-Wert größer ist als das konventionelle Signifikanzniveau von 0.01, ist es wahrscheinlich, dass dieser Wert der Teststatistik durch Zufall zustande kam falls die Nullhypothese wahr ist. Wir können demnach die Nullhypothese _nicht_ ablehnen und schließen für Beispiel 8:

> Wir können nicht ausschließen, dass die Stichprobe der Reisezeit morgens einer normalverteilten Grundgesamtheit entstammt. Die Verteilungen sind "statistisch gleich" in der klassischen Interpretation.
...
# empirische Verteilungsfunktion
cdf_e1 <- seq(0, 1-1/length(t1_sort), 1/length(t1_sort))
cdf_e2 <- seq(0, 1-1/length(t2_sort), 1/length(t2_sort))
...
Der Wert der Teststatistik $d_n$ ist diesmal 0.13; das Ausrechnen "per Hand" sparen wir uns hier. Da der p-Wert wiederum größer ist als das konventionelle Signifikanzniveau von 0.01, können wir die Nullhypothese _nicht_ ablehnen und schließen für Beispiel 9:
...
> Die Abhängigkeit zwischen "Bezirk" und "Votum" ist zwar gering, aber "statistisch signifikant" im klassischen Sinn.
