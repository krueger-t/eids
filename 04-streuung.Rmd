...
Genau genommen ist das die **korrigierte Varianz**, bei der durch $n-1$ geteilt wird und nicht durch $n$ wie man bei einer Mittelung erwarten könnte. Das Teilen durch $n-1$ garantiert eine optimale Schätzung der Varianz der Grundgesamtheit anhand der Stichprobe - mehr dazu in der schließenden Statistik. Der Nenner $n-1$ wird **Anzahl Freiheitsgrade** genannt und bezeichnet die Anzahl der Werte in einer Stichprobe, die für die Berechnung des Parameters (hier Varianz) *frei* zur Verfügung stehen, d.h. in denen sich der Zufallsprozess der Stichprobenziehung ausdrücken kann. Im Fall der Varianz ist ein Wert der Stichprobe bereits "belegt" – durch das arithmetische Mittel.^[Wenn ich das arithmetische Mittel kenne, dann kann ich einen Merkmalswert mit Hilfe der anderen und dem arithmetische Mittel ausrechnen.] Daher reduziert sich die Zahl der *freien* Elemente der Stichprobe, die in die Berechnung eingehen um eins.
...
Die Verteilungen der Merkmale "Zeit, morgens" und "Zeit, nachmittags" sind weniger rechtsschief:
...
Die Verteilung des Merkmals "Distanz" unserer Reisedaten ist spitzer als die Normalverteilung, wobei diese Variable wegen ihrer Rechtsschiefe ohnehin nicht mit der Normalverteilung vergleichbar ist. Die Verteilungen der Merkmale "Zeit, morgens" und "Zeit, nachmittags" sind etwas flacher als die Normalverteilung:
...
(\#eq:standardisierung)
\end{equation}$$
Der neue Mittelwert ist $\bar z=0$, die neue Standardabweichung ist $s_z=1$. Sollten die $x$ Daten einer Normalverteilung folgen (s. Kapitel \@ref(verteilungen)) dann folgen die $z$ Daten einer Standardnormalverteilung.
