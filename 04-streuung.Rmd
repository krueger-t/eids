...
rbeta(10000,2,3) %>% 
  as_tibble() %>% 
  ggplot(aes(x = value)) +
  stat_boxplot(geom ='errorbar', width = 0.2, col = "black", size = 2) +
  geom_boxplot(col = "black", size = 2, width = 0.4)  +
  ...
  geom_segment(aes(x = 0, y = -0.45, xend = 0.95, yend = -0.45),
               ...
               linejoin = "round", size = 2, col = "black") +
...
boxplot(reisedat$distanz, range = 0, horizontal = TRUE,
        ylim = c(0, 75), xlab = "Entfernung (km)")
...
# "Zeit, morgens" in min
# 1) arithmetisches Mittel
sbar <- mean(reisedat$zeit_morgens)
# 2) Varianz
s2s <- var(reisedat$zeit_morgens)
# 3) Standardabweichung
ss <- sd(reisedat$zeit_morgens)
# 4) Variationskoeffizient
vs <- ss / sbar
# Ergebnisse:
...
# "Zeit, morgens" in min
# 1) Schiefe
skew_s <- skewness(reisedat$zeit_morgens)
# 2) Median
med_s <- median(reisedat$zeit_morgens)
# 3) arithmetisches Mittel
mean_s <- mean(reisedat$zeit_morgens)
# Ergebnisse:
...
Die Verteilungen der Merkmale unserer Reisedaten sind alle spitzer als die Normalverteilung, wobei die Variable "Distanz" wegen ihrer Rechtsschiefe ohnehin nicht mit der Normalverteilung vergleichbar ist:

```{r echo=TRUE}
# Wölbung der "Distanz"
kurtosis(reisedat$distanz) - 3
# Wölbung der "Zeit, morgens"
kurtosis(reisedat$zeit_morgens) - 3
