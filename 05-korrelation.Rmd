...
Hier ist das Beispiel des Zusammenhangs von Grünflächenanteil und Bildungsgrad in Stadbezirken in @zimmermann2014, Kapitel 3.4.2, S. 274 - 278 sehr anschaulich. Das Merkmal "Grünflächenanteil" ist zwar metrisch skaliert, muss aber ordinal skaliert werden, um es mit dem Merkmal "Bildungsgrad" vergleichbar zu machen, das bereits ordinal vorliegt. S. Tabelle 3.36 [@zimmermann2014, S. 277].
...
An dieser Stelle können wir zu unseren Reisedaten zurückkehren. Beide Merkmale ("Distanz" und "Stationen") sind metrisch skaliert^[Die Variable "Stationen" ist jedoch eine diskrete Variable, keine kontinuierliche.] und somit können wir den **Korrelationskoeffizienten nach Bravais-Pearson** verwenden, der mehr Informationen über den Zusammenhang, nämlich **Linearität**, liefert. Einen ersten Eindruck verschafft der **Scatterplot** (auch Streudiagramm), den wir einfach mit der `plot()` Funktion in *R* generieren:
...
**Aufgabe:** _Überlegen Sie kurz, wie stark die Korrelation hier sein wird! (Auflösung weiter unten.) Überlegen Sie außerdem, woher die Streuung in "Entfernung" kommen könnte._
...
```{r echo=TRUE}
cor(reisedat$stationen, reisedat$distanz, use = "complete.obs", method = "pearson")
