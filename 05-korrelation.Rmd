...
plot(reisedat$zeit_morgens, reisedat$zeit_nachmittags, 
     xlab = "Reisezeit, morgens (min)", ylab = "Reisezeit, nachmittags (min)")

```

Hier fällt das Wertepaar $(55, 1)$ unten, in der Mitte auf. Meine Vermutung ist, dass jemand hier nachmittags $1$ im Sinne von einer Stunde eingetragen hat, anstatt $60$ Minuten. Ich ändere das in einem korrigierten Datensatz und lade diesen erneut ein (grundsätzlich muss so eine Änderung gut begründet und dokumentiert sein; alternativ kann man auch zweifelhafte Werte entfernen):
...
An dieser Stelle fallen die vier Datenpunkte ganz rechts auf, sowie der Datenpunkte unten $(14.8, 1.2)$. Bei letzterem hat sich vielleicht wieder jemand mit Minuten und Stunden vertan und meint hier $1.2$ Stunden, also $72$ Minuten. Die vier Punkte ganz rechts könnten Personen sein, die relativ weite Strecken mit dem Auto zurücklegen. Darauf schauen wir in späteren Kapitel noch etwas näher. Vorerst nutzen wir die Daten so, wie sie sind.
...
```{r echo=TRUE}
cor(reisedat$distanz, reisedat$zeit_morgens, use = "complete.obs", method = "pearson")

```
Das Argument `use = "complete.obs"` teilt R mit, nur vollständige Datenpaare zu verwenden, d.h. ohne "NA", ansonsten wäre der Output "NA". Da hier nur vollständige Datenpaare vorhanden sind, ist das Argument nur vorsorglich gesetzt. Anstatt `method = "pearson"` sind ebenfalls `method = "spearman"` und `method = "kendall"` möglich; beides sind Rangkorrelationskoeffizienten.
