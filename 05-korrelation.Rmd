...
**Aufgabe:** _Überlegen Sie kurz, wie stark die Korrelation hier sein wird! (Auflösung weiter unten.) Überlegen Sie außerdem, woher die Streuung in "Reisezeit" kommen könnte und was es mit dem einzelnen Punkt ganz rechts auf sich haben könnte._
...
Wenn wir den Punkt ganz rechts im Diagramm weglassen, ist der lineare Zusammenhang etwas stärker:

```{r echo=TRUE, out.width='80%'}
# generiere Zeitreihe ohne statistische Einheit Nr. 6
...
```{r echo=TRUE}
cor(reisedat2$distanz, reisedat2$zeit_morgens,
    use = "complete.obs", method = "pearson")

```

Datenpunkte wegzulassen ist gut zu begründen und in jedem Fall transparent zu machen! Wir werden das im Seminar diskutieren.
