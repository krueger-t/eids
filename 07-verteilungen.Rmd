...
In Kapitel \@ref(wahrscheinlichkeit) haben wir gesehen, dass alle 36 Elementarereignisse zweier Würfelwürfe $\omega=(1;1),(1;2),\ldots,(6;6)$ (vgl. Abbildung \@ref(fig:wuerfel2)) die gleiche Wahscheinlichkeit haben, nämlich $\Pr(\omega_i)=\frac{1}{6}\cdot\frac{1}{6}=\frac{1}{36}$. In Abbildung \@ref(fig:prverteilung), links sehen wir eine Verteilung dieser Wahrscheinlichkeiten; in diesem Fall ist es eine sogenannte _diskrete Gleichverteilung_. Im Quiz haben wir gesehen, dass wenn uns das Ereignis "Gesamtaugenzahl" interessiert, hier mit $E$ symbolisiert, es Ergebnisse gibt die wahrscheinlicher sind als andere (das deckt sich mit unserer Erfahrung mit Würfelspielen); die Verteilung ist symmetrisch um ein Maximum bei "7" (Abbildung \@ref(fig:prverteilung), rechts), eine sogenannte _diskrete Dreiecksverteilung_. Formal schreiben wir:
$$\begin{equation}
...
Wir haben bereits Darstellungen von **empirischen Verteilungen** in der Form von Histogrammen (und Boxplots) kennengelernt (vgl Kapitel \@ref(haeufigkeit)). Nehmen wir das Beispiel "Entfernung" aus Ihren Reisedaten (Abbildung \@ref(fig:fFempirisch)). "Entfernung" ist eine stetige Zufallsvariable, die aber hier klassiert dargestellt ist. Ein Beispiel für eine diskrete Zufallsvariable ist das Merkmal "Anzahl Vulkanausbrüche" aus @zimmermann2014.

```{r fFempirisch, echo=FALSE, fig.align='center', fig.cap='Links: Empirische **Verteilung** des Merkmals "Entfernung". Der 5. Balken z.B. hat eine Höhe von etwa 0.05; d.h. rund 5% der Merkmalswerte sind in Klasse 5 (20-25km). Rechts: Empirische **Verteilungsfunktion** des Merkmals "Entfernung". Der 5. Balken hat eine Höhe von etwa 0.88; d.h. rund 88% der Merkmalswerte sind kleiner oder gleich 25km.', fig.show='hold', out.width='50%'}
...
curve(dlnorm(x, meanlog=mean(log(reisedat$distanz)), sdlog=sd(log(reisedat$distanz))), add=TRUE, col = "red")
plot(h2, freq = TRUE, col = "gray",
     main = "", xlab = "Entfernung (km)", ylab = "Verteilungsfunktion")
curve(plnorm(x, meanlog=mean(log(reisedat$distanz)), sdlog=sd(log(reisedat$distanz))), add=TRUE, col = "red")
```
...
> In Worten: Die Wahrscheinlichkeitsfunktion einer diskreten Zufallsvariablen $X$ in Abhängigkeit der Ausprägung $x$ ist die Eintrittswahrscheinlichkeit von $x$. Vgl. Abbildung \@ref(fig:wuerfel2) und Abbildung \@ref(fig:prverteilung), links.
...
> In Worten: Das Integral von $x=a$ bis $x=b$ der Dichtefunktion einer stetigen Zufallsvariablen $X$ in Abhängigkeit der Ausprägung $x$ ist die Wahrscheinlichkeit, dass $X$ zwischen $a$ und $b$ liegt. Vgl. Abbildung \@ref(fig:fFtheoretisch), links.
...
> In Worten: Die Verteilungsfunktion einer beliebigen Zufallsvariablen $X$ in Abhängigkeit der Ausprägung $x$ ist die Wahrscheinlichkeit, dass $X$ kleiner oder gleich $x$ ist. Vgl. Abbildung \@ref(fig:fFtheoretisch), rechts.
...
## Parameter und Kenngrößen theoretischer Verteilungen
...
Schauen wir uns das für den einfachsten Fall der sogenannten _Normalverteilung_ an (Abbildung \@ref(fig:npdf)).^[Der Name Normalverteilung heisst nicht, dass diese Verteilung im umgangssprachlichen Sinn "normaler" ist als andere Verteilungen. Sie ist jedoch, wie wir noch sehen werden, tatsächlich ein gutes Modell für viele Zufallsvorgänge.]
```{r npdf, echo=TRUE, fig.align='center', fig.cap='Form der Dichtefunktion der Normalverteilung einer beliebigen Zufallsvariablen $X$. Hier ist die **Standardnormalverteilung** $N(0,1)$ dargestellt, mit Mittelwert $\\mu=0$ und Standardabweichung $\\sigma=1$.', out.width='80%'}
# generiere x-Werte von -5 bis 5 in 0.1-Schritten
x <- seq(-5,5,0.1)
# plotte Dichtefunktion der Normalverteilung mit mu=0 und sigma=1 für x
...
```{r npdfvariation, echo=TRUE, fig.align='center', fig.cap='Links: Dichtefunktion der Normalverteilung einer beliebigen Zufallsvariablen $X$ für verschiedene Werte $\\mu$. Wir sehen, dass der Parameter $\\mu$ die Verteilung nach rechts bzw. links verschiebt. Rechts: Dichtefunktion der Normalverteilung einer beliebigen Zufallsvariablen $X$ für verschiedene Werte $\\sigma$. Wir sehen, dass der Parameter $\\sigma$ die Verteilung breiter bzw. schmaler macht.', fig.show='hold', out.width='50%'}
# generiere x-Werte von -5 bis 5 in 0.1-Schritten
x <- seq(-5,5,0.1)
# plotte Dichtefunktion der Normalverteilung mit mu=-1, 0, 1 und sigma=1 für x
...
# plotte Dichtefunktion der Normalverteilung mit mu=0 und sigma= 1, 1.5, 2 für x
plot(x, dnorm(x, mean=0, sd=1), type='l', ylim=c(0,0.45),
     xlab='X=x', ylab='Dichtefunktion', panel.first = grid())
...
```{r ncdfvariation, echo=TRUE, fig.align='center', fig.cap='Links: Verteilungsfunktion der Normalverteilung einer beliebigen Zufallsvariablen $X$ für verschiedene Werte $\\mu$. Rechts: Verteilungsfunktion der Normalverteilung einer beliebigen Zufallsvariablen $X$ für verschiedene Werte $\\sigma$. Vgl. Abbildung \\@ref(fig:npdfvariation).', fig.show='hold', out.width='50%'}
# generiere x-Werte von -5 bis 5 in 0.1-Schritten
x <- seq(-5,5,0.1)
# plotte Verteilungsfunktion der Normalverteilung mit mu=-1, 0, 1 und sigma=1 für x
...
legend('topleft', legend=c('N(-1,1)', 'N(0,1)', 'N(1,1)'),
       col=c('red', 'black', 'blue'), lty=1)
# plotte Verteilungsfunktion der Normalverteilung mit mu=0 und sigma= 1, 1.5, 2 für x
plot(x, pnorm(x, mean=0, sd=1), type='l',
     xlab='X=x', ylab='Verteilungsfunktion', panel.first = grid())
...
Kommen wir nun zu den **Kenngrößen** theoretischer Verteilungen. Wiederum ist wichtig, dass wir die Parallelen zu den empirischen Verteilungen sehen.
...
Die Formulierung der Varianz ist analog zur Formulierung der Varianz empirischer Verteilungen (vgl. Kapitel \@ref(streuung)):
...
> In Worten: Das $p$-Quantil einer beliebigen Zufallsvariablen $X$ ist die Stelle $x_p$, an der die Verteilungsfunktion $F_X$ der Zufallsvariablen gleich $p$ ist. Die Verteilungsfunktion gibt die Wahrscheinlichkeit an, dass die Zufallsvariable $X$ einen Wert kleiner oder gleich $x_p$ annimmt.
...
```{r grenzwertsatz, echo=TRUE, fig.align='center', fig.cap='Illustration des zentralen Grenzwertsatzes. Mit zunehmender Anzahl addierter Zufallsprozesse (hier Würfelwürfe, die jeder einer Gleichverteilung folgen) nähert sich die Wahrscheinlichkeitsverteilung der Summe einer Normalverteilung an.', fig.show='hold', message=FALSE, warning=FALSE, out.width='33%'}
...
Die Parameter der Lognormalverteilung sind, leider wie die der Normalverteilung, ebenfalls mit $\mu$ und $\sigma$ bezeichnet. Diese sind aber nicht mehr gleich mit Erwartungswert und Standardabweichung der Zufallsvariablen; hier sind es lediglich Symbole für die Parameter. Die Symbole haben sich etabliert weil es eine einfache Entsprechung zwischen Lognormal- und Normalverteilung gibt: Wenn $X$ lognormalverteilt ist, $X\sim logN(\mu,\sigma)$, dann ist das log-transformierte $X$ normalverteilt, $\log(X)\sim N(\mu,\sigma)$, mit den gleichen Parametern $\mu$ und $\sigma$. D.h. die Schätzer für die beiden Parameter sind das arithmetische Mittel bzw. die Standardabweichung der _log-transformierten_ Daten einer Stichprobe (s. Kapitel \@ref(schaetzen)).

Schauen wir uns die Form der Lognormalverteilung für verschiedene Parameterkonstellationen an (Abbildung \@ref(fig:lognvariation)): Beide Parameter kontrollieren hier sowohl den Erwartungswert als auch die Varianz und damit auch die Form der Verteilung; im Beispiel sehen wir das nur für $\sigma$ während $\mu$ konstant gehalten ist. An den Formeln für Erwartungswert und Varianz könnten wir sehen, dass dort jeweils beide Parameter eingehen. Das können Sie beisielsweise in @mittag2016 nachlesen.
...
x <- seq(0,3,0.05)
# plotte Dichtefunktion der Lognormalverteilung mit mu=0 und sigma= 0.25, 0.5, 1 für x
...
Da unsere Entfernungsdaten rechtsschief verteilt sind mit relativ kleinen Werten (wenige Kilometer) und relativ großen Werten (Zehner von Kilometern) bietet sich die Lognormalverteilung als theoretische Verteilung an und lässt sich auch gut an die Daten anpassen (vgl. Abbildung \@ref(fig:fFempirisch)). Interessanterweise waren die Entfernungsdaten aus dem vergangenen Semester eher normalverteilt, ohne die Rechtsschiefe, die wir in diesem Semester sehen. Vielleicht liegt das daran, dass aufgrund der Corona-bedingten digitalen Lehre einige von Ihnen außerhalb von Berlin wohnen, da sie nicht nach Adlershof pendeln müssen.

Nutzen wir jedenfalls die Entfernungsdaten, um uns den Zusammenhang zwischen Lognormal- und Normalverteilung zu veranschaulichen. Wir hatten gesagt, wenn $X$ lognormalverteilt ist (Abbildung \@ref(fig:lognn), links) dann ist $\log(X)$ normalverteilt mit den gleichen Parametern; das sehen wir in Abbildung \@ref(fig:lognn), rechts.
...
# Histogramm berechnen ohne Output
hlog <- hist(log(reisedat$distanz), breaks = seq(0, 5, 0.5), plot = FALSE)
# absolute in relative Häufigkeiten umrechnen
...
Die **diskrete** Gleichverteilung haben wir bereits kennengelernt (hier nochmal in Abbildung \@ref(fig:u), links). Beispiele sind die Elementarereignisse von Glückspielen: Würfelwürfe, Münzwürfe, Roulette, Lottozahlen usw. Analog dazu gibt es auch eine **stetige** Gleichverteilung (Abbildung \@ref(fig:u), rechts):
$$\begin{equation}
X\sim U(a,b)
(\#eq:xuverteilt)
\end{equation}$$
...
plot(seq(0,40,1), dbinom(seq(0,40,1), 20, 0.5), ylim=c(0,0.2), type='p',
     xlab='X=x', ylab='Wahrscheinlichkeitsfunktion')
points(seq(0,40,1), dbinom(seq(0,40,1), 20, 0.7), col='red')
points(seq(0,40,1), dbinom(seq(0,40,1), 40, 0.5), col='blue')
...
points(seq(0,40,1), pbinom(seq(0,40,1), 20, 0.7), col='red')
points(seq(0,40,1), pbinom(seq(0,40,1), 40, 0.5), col='blue')
legend('bottomright', legend=c('B(0.5,20)', 'B(0.7,20)', 'B(0.5,40)'),
       col=c('black', 'red', 'blue'), pch=1)
```

In der Geographie verwenden wir die Binomialverteilung häufig für _Zähldaten_ und _Proportionen_, z.B. in der Biogeographie die Anzahl Individuen einer bestimmten Tierart in einem Gebiet oder der Anteil überlebender Organismen als Funktion der Konzentration einer toxischen Substanz. Die Binomialverteilung dient aber auch als Wahrscheinlichkeitsmodell für _Präsenz bzw. Absenz_ beispielsweise einer Tierart in einem Gebiet. Dazu mehr im Masterstudiengang _Global Change Geography_.

### Poisson-Verteilung

Die Poisson-Verteilung ist eine weitere _diskrete_ Verteilung für _Zähldaten_, z.B. die Anzahl Vogelarten in einem Gebiet als Funktion topographischer Parameter.^[Auch das werden wir uns im Masterstudiengang _Global Change Geography_ näher anschauen.] Wir schreiben:
$$\begin{equation}
X\sim Pois(\lambda)
(\#eq:xpoisverteilt)
\end{equation}$$
...
plot(seq(0,20,1), dpois(seq(0,20,1), 1), ylim=c(0,0.4), type='p',
     xlab='X=x', ylab='Wahrscheinlichkeitsfunktion')
points(seq(0,20,1), dpois(seq(0,20,1), 4), col='red')
points(seq(0,20,1), dpois(seq(0,20,1), 10), col='blue')
legend('topright', legend=c('Pois(1)', 'Pois(4)', 'Pois(10)'),
       col=c('black', 'red', 'blue'), pch=1)
# cdf
plot(seq(0,20,1), ppois(seq(0,20,1), 1), ylim=c(0,1), type='p',
     xlab='X=x', ylab='Verteilungsfunktion')
points(seq(0,20,1), ppois(seq(0,20,1), 4), col='red')
points(seq(0,20,1), ppois(seq(0,20,1), 10), col='blue')
