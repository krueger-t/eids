...
plot(reisedat$distanz, reisedat$zeit, 
     xlab = "Entfernung (km)", ylab = "Reisezeit (min)")
```

Wir erinnern uns, dass der Korrelationskoeffizient nach Bravais-Pearson aus Kapitel \@ref(korrelation) 0.75 war. Das Ziel ist nun, eine Gerade durch die Punktwolke zu legen, die den *Trend* beschreibt, so dass der Abstand der Punkte von der Geraden minimal ist.
...
tibble( x = c(-0.407, 1, 2, 3, 3.5, 4, 5, 6, 7, 8),
  ...
  theme(axis.text.x = element_blank(),
        ...
        axis.line.y = element_line(arrow = grid::arrow(length = unit(0.25, "cm")),size = 1.5),
        axis.line.x = element_line(arrow = grid::arrow(length = unit(0.25, "cm")),size = 1.5),
  ...
  geom_segment(aes(x=-0.9,xend=-0.4,y=2.72,yend=2.72), linetype = 3, size = 1) +
  geom_segment(aes(x= 2,xend= 2, y=2.94, yend=3), linetype = 3, size = 1) +
  geom_segment(aes(x=4.3,xend=4,y=3.15,yend=3.3), linetype = 3, size = 1) +
  geom_segment(aes(x=4,xend=5.5,y=3.1,yend=3.1), linetype = 3, size = 1) +
  geom_segment(aes(x=5.5,xend=5.5,y=3.1,yend=3.245), linetype = 3, size = 1) +
...
reise_fit <- lm(zeit ~ distanz, data = reisedat)
# Informationen über geschätzte Parameterwerte ausgeben
coef(summary(reise_fit))
```
In der ersten Spalte ("Estimate") dieses Outputs finden Sie die Werte der Parameterschätzer, wobei "(Intercept)" für $\beta_0$ steht und "distanz" (in diesem Fall) für $\beta_1$. Anhand von $\beta_1$ können wir ablesen, dass sich pro km Entfernung die Reisezeit um 2min erhöht. Der Achsenabschnitt $\beta_0$ hat keine direkte Entsprechung.^[Wenn wir jedoch die unabhängige Variable "distanz" zentrieren würden, d.h. von allen Datenpunkten den Mittelwert abziehen würden, dann ergäbe sich ein Achsenabschnitt, der die Entfernung für die mittlere Entfernung darstellen würde.] Auf die anderen Spalten werden wir weiter unten zu sprechen kommen. Plotten wir nun die so ermittelte Regressionsgerade $y_i=21.5+2.0\cdot x_i+\epsilon_i$:
...
```
D.h. rund 57% der Varianz in den Reisezeit-Daten wird durch das lineare Modell mit Entfernung als Prädiktor erklärt.
...
Dieser Datensatz zeigt, außer dem Ausreißer oben links, kein erkennbares Muster, was für eine Unabhängigkeit der Residuen spricht.
...
Diese Grafik zeigt eine leichte Zunahme der Residuenvarianz von links nach rechts, d.h. in Richtung größerer Werte - eine Form der Heteroskedastizität - wobei das aufgrund der wenigen sehr großen Werte der Reisezeit schwer zu sagen ist.
...
Hier fällt wiederum der Ausreißer auf. Ansonsten scheinen die Residuen normalverteilt. Ein Histogramm der Residuen ist eine weitere nützliche Darstellung:
```{r echo=TRUE}
# Histogramm der Residuen plotten
hist(residuals(reise_fit), xlim = c(-70,70))
```

Der eine Ausreißerdatenpunkt scheint also nicht mit dem restlichen Zusammenhang in den Daten zusammenzupassen. Dies ist ein Grund, ihn rauszulassen; eine solche Entscheidung muss aber immer gut begründet und transparent getroffen werden! Wiederholen wir also die Regression ohne besagten Ausreißer:

```{r echo=TRUE}
# Eliminierung des Ausreißers
reisedat2 <- reisedat
reisedat2[reisedat$zeit > 120,] <- c(NA, NA)
# lineare Regression der Reisedaten
reise_fit2 <- lm(zeit ~ distanz, data = reisedat2)
# Informationen über geschätzte Parameterwerte ausgeben
...
```{r echo=TRUE}
# Modellanpassung plotten
plot(reisedat2$distanz, reisedat2$zeit,
     pch = 19, type = 'p',
     xlab = "Entfernung (km)", ylab = "Reisezeit (min)")
...
```{r echo=TRUE}
# Residuen gegen Index plotten
plot(residuals(reise_fit2), pch = 19, type = 'p', ylim = c(-30,30))
abline(h = 0, lwd = 3, col = "red")
```

```{r echo=TRUE}
# Residuen gegen modellierte Werte von y plotten
plot(fitted.values(reise_fit2),residuals(reise_fit2), pch = 19, type = 'p', ylim = c(-30,30))
abline(h = 0, lwd = 3, col = "red")
```
...
```{r echo=TRUE}
# Histogramm der Residuen plotten
hist(residuals(reise_fit2), xlim = c(-30,30))
```

Ohne den Ausreißer ist die erklärte Varianz deutlich besser (70%) und die Residuenannahmen sind besser erfüllt, abgesehen von der leichten Heteroskedastizität. Die Parameterschätzer sind ähnlich (da es sich nur um einen Ausreißer handelte).
