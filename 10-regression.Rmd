...
reisedat2 <- reisedat[-c(13),]
reisedat3 <- reisedat[-c(13,30,37,63,71),]
plot(reisedat$distanz, reisedat$zeit_morgens, 
     xlab = "Entfernung (km)", ylab = "Reisezeit, morgens (min)", main = "Originaldaten")
plot(reisedat2$distanz, reisedat2$zeit_morgens, 
     xlab = "Entfernung (km)", ylab = "Reisezeit, morgens (min)", main = "ohne Punkt (14.8,1.2)")
plot(reisedat3$distanz, reisedat3$zeit_morgens, 
     xlab = "Entfernung (km)", ylab = "Reisezeit, morgens (min)", main = "ohne Punkt (14.8,1.2) und 4 Punkte ganz rechts")
```

Wir erinnern uns, dass der Korrelationskoeffizient nach Bravais-Pearson aus Kapitel \@ref(korrelation) 0.82 war (für die Originaldaten). Das Ziel ist nun, eine Gerade durch die Punktwolke zu legen, die den *Trend* beschreibt, so dass der Abstand der Punkte von der Geraden minimal ist.
...
Wir setzen nun Gleichung \@ref(eq:b13) in Gleichung \@ref(eq:b04) ein und haben eine exakte Lösung für $\hat\beta_0$. Berechnen wir nun die Parameter für unsere drei Reisedatenverianten mit der `lm()` Funktion:
...
plot(reisedat3$distanz, reisedat3$zeit_morgens,
     pch = 19, type = 'p', xlim = c(0,80), ylim = c(0,140),
     xlab = "Entfernung (km)", ylab = "Reisezeit, morgens (min)")
abline(coef(reise_fit3), lwd = 3, col = "red")
```
...
abline(h = 0, lwd = 3, col = "red")
plot(residuals(reise_fit3), pch = 19, type = 'p', ylim = c(-70,70))
abline(h = 0, lwd = 3, col = "red")
```
...
abline(h = 0, lwd = 3, col = "red")
plot(fitted.values(reise_fit3),residuals(reise_fit3), pch = 19, type = 'p', ylim = c(-70,70))
abline(h = 0, lwd = 3, col = "red")
```
...
qqnorm(residuals(reise_fit3))
qqline(residuals(reise_fit3))
```
...
hist(residuals(reise_fit3), xlim = c(-70,70))
```
