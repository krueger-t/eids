...
Das ist eine ungeordnete Reihe von `r length(reisedat$distanz)` Datenpunkten, den sogenannten **Rohdaten**.
...
Die Häufigkeiten der Klassen sind hier kumuliert aufsummiert, d.h. der letzte Balken ganz rechts hat die absolute Höhe `r length(reisedat$distanz)`, die Gesamtzahl der Datenpunkte $n$, bzw. die relative Höhe 1 (100%). Sehen die dazu auch das Beispiel in @zimmermann2014, Tabelle 3.10 auf Seite 87.
...
2. Ein Klasse für den Modus ("Modalklasse") angeben, z.B. für unsere Entfernungsdaten: "Bei einer Klassierung der Entfernungsdaten mit Klassenbreite 5km ist die häufigste Klasse $(5;10]$."
...
Ist das Produkt $n\cdot p$ _ganzzahlig_, dann ist $j=n\cdot p$ und:
