...
Schauen wir uns dazu Ihre Reisedaten an, und zwar nur die Entfernungen (der _R_-Code wird in den PC-Übungen schnell klar werden):
...
reisedat$distanz
```
Das ist eine ungeordnete Reihe von 74 Datenpunkten, die sogenannten **Rohdaten**.
...
Das Histogramm bleibt gleich, nur dass die vertikale Achse anders skaliert ist. Da das in der `hist()` Funktion  nicht vorgesehen ist, ist der _R_-Code etwas länger:
...
Abschließend noch ein paar Worte zur **Klassifizierung**. Äquidistante Klassen, d.h. Klassen gleicher Breite, sind grundsätzlich zu bevorzugen. In _R_ können Sie eine gewünschte Anzahl Klassen angeben, die dann äquidistant über den Wertebereich verteilt werden. Das ist sinnvoll für einen ersten Eindruck. Die Grundeinstellung von 10 Klassen ist dabei meist ausreichend. Im Laufe der Analyse wird es manchmal sinnvoller sein, bestimmte Klassen vorzugeben, auch (oder gerade) wenn manche Klasse in der betrachteten Stichprobe nicht vorkommen. Auch das ist in _R_ möglich, in dem Sie die Klassengrenzen ("breaks") angeben. Probieren wir das in _R_ aus:
